...
classdef SIM_EXPERIMENT_cnn2_cells < handle
   properties     
       % image parameters
       sl_em
       sl_ex 
   ...
   methods
       % constructor
       function obj = SIM_EXPERIMENT_cnn2_cells(parameterFile)
       ...
       function self = set_X0(self,maxCounts,X0_in) 
           self.X0_sig = maxCounts;
           if isempty(X0_in) % default is beads simulations 
               self.X0.Data = gen_beadsIn2D(self.Nx,self.Ny,0)*self.X0_sig; 
           else
               self.X0.Data = X0_in*self.X0_sig; 
