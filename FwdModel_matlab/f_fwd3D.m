...
function [Yhat Xgt] = f_fwd3D(X0,E,PSFs,pram)
  ...
  % Dimentionality needed (here t = patterns,b = instances) 
  %  X0          - [y,x,z  ,t=1 ,b  ]
  %  E           - [y,x,z=1,t=Nt,b=1]
  ...
  X0        = padarray(X0,round([(size(exPSF,1)-pram.Ny)/2 (size(exPSF,2)-pram.Nx)/2 0 0]),0,'both');
  X0        = X0(1:size(exPSF,1),1:size(exPSF,2),1:size(exPSF,3),:);
  ...
  for i=1:size(X_ex,4)                              % using the loop in the function is slow for some reason
    X_sctterd(:,:,:,i) = f_conv2nd(sPSF ,X_ex(:,:,:,i),'same');
  end
  % X_em      = f_conv3nd(emPSF,X_sctterd,'same');  % this is slower than looping outside the function as below
  for i=1:size(X_ex,4)                              % using the loop in the function is slow for some reason
    X_em(:,:,:,i)      = f_conv3nd(emPSF,X_sctterd(:,:,:,i),'same');
  end
